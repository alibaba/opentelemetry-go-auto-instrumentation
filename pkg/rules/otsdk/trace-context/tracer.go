...
func (tr *tracer) newSpan(ctx context.Context, name string, config *trace.SpanConfig) trace.Span {
	...
	if config.NewRoot() {
		...
		if p := trace.SpanFromContext(ctx); p == nil || !p.SpanContext().IsValid() {
			if tc := getOrInitTraceContext(); tc.size() != 0 {
				psc = tc.tail().SpanContext()
			}
		} else {
	...
	if !isRecording(samplingResult) {
		span := tr.newNonRecordingSpan(sc)
		traceContextAddSpan(span)
		return span
	}
	span := tr.newRecordingSpan(psc, sc, name, samplingResult, config)
	traceContextAddSpan(span)
	return span
}
