...
func main() {
	...
	client := http.Client{}
	client.Get("http://localhost:8080/helloworld/kratos")
	fmt.Printf("Send http request to kratos")

	verifier.WaitAndAssertTraces(func(stubs []tracetest.SpanStubs) {
		protocolType := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.protocol.type").AsString()
		if protocolType != "http" {
			panic("protocol type should be grpc, actually got " + protocolType)
		}
		serviceName := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.service.name").AsString()
		...
		serviceId := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.service.id").AsString()
		if serviceId != "opentelemetry-id" {
			panic("service id should be opentelemetry-id, actually got " + serviceId)
		}
		serviceVersion := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.service.version").AsString()
		if serviceVersion != "v1" {
			panic("service version should be v1, actually got " + serviceVersion)
		}
		serviceMetaAgent := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.service.meta.agent").AsString()
		if serviceMetaAgent != "opentelemetry-go" {
			panic("service meta agent should be opentelemetry-go, actually got " + serviceMetaAgent)
		}
		serviceEndpoint := verifier.GetAttribute(stubs[0][1].Attributes, "kratos.service.endpoint").AsStringSlice()
