...
func main() {
	...
	verifier.WaitAndAssertTraces(func(stubs []tracetest.SpanStubs) {
		...
		verifier.VerifyDbAttributes(stubs[3][0], "select users", "mysql", "127.0.0.1", "select name from users where id = ?", "select", "users")
		verifier.VerifyDbAttributes(stubs[4][0], "select users", "mysql", "127.0.0.1", "select name from users where id = ?", "select", "users")
	}, 5)
}
